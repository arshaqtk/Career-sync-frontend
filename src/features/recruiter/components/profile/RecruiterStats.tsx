      ...
      {stats.map((s) => (
        <Card key={s.label}>
          <CardContent className="p-6 flex flex-col items-center justify-center gap-1.5">
            <p className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest text-center">{s.label}</p>
            <p className="text-2xl font-black text-foreground text-center leading-none">{s.value}</p>
          </CardContent>
        </Card>
      ))}
    </div>
